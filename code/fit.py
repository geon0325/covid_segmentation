...
def nl_fit(nlds, ftype, wtype, dps): 
    #--------------------------------#
    if(ftype!='m0' and ftype!='A2'and ftype!='A01' and ftype!='B01'): 
        tl.warning("fit.py: usage: [m0/M2/A01/B01]")
    #--------------------------------#
    if(ftype=='m0'): # if m0, i.e., arg{s(0)=m0}
...
def _createP(nlds, ftype):
   ...
   #--------------------------------------------------#
    ...
    #--------------------------------------------------#
        for i in range(0,k):
            P.add('m0_%i'%(i), value=nlds.m0[i], vary=V)
   #--------------------------------------------------#
    if(ftype=='A2'):
    #--------------------------------------------------#
        for i in range(0,k):
