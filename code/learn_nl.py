...
def _train_nl(X, k=2, linearFit=False):
    model = NLDS(X, k)
...
def segment(X, BOUND, linearFit, N, k, max_bound=-1e10):
    s = BOUND[0]; e = BOUND[1];
    (n, d) = np.shape(X[s:e])
    
    # without segmentation
    model = _train_nl(X[s:e], k, linearFit=linearFit)
    ...
    for i in range(2, n-2):
        ...
        try:
            model_1 = _train_nl(X[s1:e1], k, linearFit=linearFit)
            dist_1 = _get_dist(model_1, linearFit=linearFit)[1]
            costM_1 = costM
            costC_1, std_1 = get_costC(X[s1:e1], dist_1, std)

            model_2 = _train_nl(X[s2:e2], k, linearFit=linearFit)
    ...
    if min_costT == costT:
        ...
        min_DIST_1, min_BOUND_1, min_MODEL_1 = segment(X, min_BOUND[0], linearFit, N, k)
        min_DIST_2, min_BOUND_2, min_MODEL_2 = segment(X, min_BOUND[1], linearFit, N, k)
        return min_DIST_1 + min_DIST_2, min_BOUND_1 + min_BOUND_2, min_MODEL_1 + min_MODEL_2

def fit_data(X, fn, linearFit, mean, std, k):
    #----------------------------------#
    ...
    DIST, BOUND, MODEL = segment(X, [0, n], linearFit, n, k)
...
def forecast_data(X, fn, linearFit, mean, std, k):
    #----------------------------------#
    tl.comment("Start learning NL")
    #----------------------------------#
    ...
    DIST, BOUND, MODEL = segment(X, [0, train_size], linearFit, train_size, k, max_bound=train_size-test_size)
    BOUND = [[BOUND[i],BOUND[i+1]] for i in range(0, len(BOUND), 2)]
...
def fit_data_single(X, fn, linearFit, mean, std, k):
    #----------------------------------#
    tl.comment("Start learning NL")
    #----------------------------------#
    ...
    model = _train_nl(X, k, linearFit=linearFit)
    dist = _get_dist(model, linearFit=linearFit)[1]
    DIST = [dist]
    BOUND = [[0, n]]
...
def forecast_data_single(X, fn, linearFit, mean, std, k):
    #----------------------------------#
    tl.comment("Start learning NL")
    #----------------------------------#
    ...
    model = _train_nl(X[:train_size], k, linearFit=linearFit)
...
def fit_data_inc(X, fn, ERR_RAT, linearFit, mean, std, k):
    #----------------------------------#
    tl.comment("Start learning NL")
    #----------------------------------#
    ...
    while e_idx < n:
        ...
        try:
            model = _train_nl(X[s_idx:e_idx], k, linearFit=linearFit)
            dist = _get_dist(model, linearFit=linearFit)[1]
